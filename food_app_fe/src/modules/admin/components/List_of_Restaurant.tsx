...
import { RestaurantData } from "../../../Types/admin";
...
const Restaurant: React.FC = () => {
    ...
    const fetchUsers = async (page: number) => {
        const url: string = searchval ? `admin/getrestaurants/${page}/${searchval}` : `admin/getrestaurants/${page}/nothing`;
    ...
    return (
        <div>
            <p className=" text-center font-bold text-3xl text-red-600 ">Restaurants</p>
            ...
            <div className="flex justify-center">
                <table className=" table-auto mt-10 max-w-[1000px] w-[700px] border-collapse border border-slate-500 rounded">
                    ...
                    <tbody>
                        {restaurants && restaurants.map((data: RestaurantData) => (
                            <tr key={data.id} className="p-4">
                                ...
                                <td className="py-6 px-6 border border-slate-700 text-center">{data.name}</td>
                                <td className="py-4 px-6 border border-slate-700 text-center">{data.phone}</td>
