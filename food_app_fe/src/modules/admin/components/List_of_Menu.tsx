...
import { MenuData } from "../../../Types/admin";
...
const Menu: React.FC = () => {
    ...
    const fetchUsers = async (page: number) => {
        const url: string = searchval ? `admin/getmenus/${page}/${searchval}` : `admin/getmenus/${page}/nothing`;
    ...
    return (
        <div>
            <p className=" text-center font-bold text-3xl text-red-600 ">MenuData</p>
            ...
            <div className="flex justify-center">
                <table className=" table-auto mt-10 max-w-[1000px] w-[700px] border-collapse border border-slate-500 rounded">
                    ...
                    <tbody>
                        {menus && menus.map((data: MenuData) => (
                            <tr key={data.id} className="p-4">
                                <td className="py-6 px-6 border border-slate-700 text-center">{data.id}</td>
                                <td className="py-6 px-6 border border-slate-700 text-center">{data.restaurant_id}</td>
                                <td className="py-6 px-6 border border-slate-700 text-center">{data.item_name}</td>
                                <td className="py-4 px-6 border border-slate-700 text-center">{data.price}</td>
